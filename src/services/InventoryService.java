...
public class InventoryService {
    ...
    private Item putItemBody(Player player, Item item) {
        ...
        // Chỉ cho phép các loại trang bị hợp lệ
        ...
        if (player.isPet) {
            boolean isNormalType = item.template.type >= 0 && item.template.type <= 5;
            boolean isPetSpecialItem
                    = item.template.id == 1828
                    || item.template.id == 1893
                    || item.template.id == 1835
                    || item.template.id == 1966;
            boolean allowType32ForPet = item.template.type == 32;

            if (!(isNormalType || isPetSpecialItem || allowType32ForPet)) {
        ...
        if (!player.isPet
                ...
                || item.template.id == 1835
                || item.template.id == 1966)) {
            Service.gI().sendThongBao(player, "Vật phẩm này chỉ dành cho đệ tử!");
            return sItem;
        }
        ...
        for (Item.ItemOption io : item.itemOptions) {
            if (io.optionTemplate.id == 21) {
                powerRequire = io.param * 1_000_000_000L;
                break;
            }
        }
        ...
        if (index < 0) {
            Service.gI().sendThongBaoOK(
                    player.isPet ? ((Pet) player).master : player,
                    "Không xác định được ô trang bị!");
            return sItem;
        }

        // CHẶN Ô CHƯA MỞ
        if (!player.isPet) {
            if (!player.isBodySlotUnlocked(index)) {
                Service.gI().sendThongBao(player, "Hãy tiến cấp để mặc ô trang bị này!!");
                return sItem;
            }
        } else {
            // Đệ tử: đảm bảo index không vượt quá số ô body hiện có
            if (index >= player.inventory.itemsBody.size()) {
                Service.gI().sendThongBaoOK(
                        ((Pet) player).master,
                        "Đệ tử chưa mở ô trang bị này!");
                return sItem;
            }
        }
            ...
            case Inventory.TYPE_NORMAL_BOX -> {
                ...
                if (item.isNotNullItem()) {
                    ...
                    if (item.template.type >= 0 && item.template.type <= 5) {
                        int bodyIndex = item.template.type;
...
//                return;
//            }
//            if (item.template.id == 1705) {
//                Service.gI().sendThongBao(player, "Không thể xu elite vào rương");
//                return;
//            }
    ...
    public void sendItemBody(Player player) {
        ...
        try {
            ...
            int openedSlots;
            if (player.isPet) {
                openedSlots = player.inventory.itemsBody.size();
            } else {
                openedSlots = Inventory.BASE_BODY_SLOT_COUNT + player.bodySlotExtra;

                if (openedSlots < Inventory.BASE_BODY_SLOT_COUNT) {
                    openedSlots = Inventory.BASE_BODY_SLOT_COUNT;
                }
                if (openedSlots > Inventory.BODY_SLOT_COUNT) {
                    openedSlots = Inventory.BODY_SLOT_COUNT;
                }
            }

            msg.writer().writeByte(openedSlots);

            for (int i = 0; i < openedSlots; i++) {
    ...
    public boolean addItemList(List<Item> items, Item itemAdd) {
        ...
        if (itemAdd.template.isUpToUp) {
            for (Item it : items) {
                ...
                if (it.quantity < 99_999) {
                    int add = 99_999 - it.quantity;
    ...
    public boolean findItemBongTaiCap3(Player player) {
        for (Item item : player.inventory.itemsBag) {
            if (item.isNotNullItem() && item.template.id == 1774) {
                return true;
            }
        }
        for (Item item : player.inventory.itemsBox) {
    ...
    public boolean hoansec(Player player) {
    ...
    public Item findItemInAllInventories(Player player, int tempId) {
        Item bodyItem = this.findItem(player.inventory.itemsBody, tempId);
        if (bodyItem != null) {
            return bodyItem;
        }
        Item bagItem = this.findItem(player.inventory.itemsBag, tempId);
        if (bagItem != null) {
            return bagItem;
        }
        Item boxItem = this.findItem(player.inventory.itemsBox, tempId);
        if (boxItem != null) {
            return boxItem;
        }
        return null;
