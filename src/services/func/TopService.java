    ...
    public void updateTop() {
        if (Manager.timeRealTop + (10 * 60 * 1000) < System.currentTimeMillis()) {
            ...
            try (Connection con = DBConnecter.getConnectionServer()) {
                ...
                Manager.topPhaoBong = Manager.realTop(ConstSQL.TOP_PHAO_BONG, con);
                Manager.topLixi = Manager.realTop(ConstSQL.TOP_LIXI, con);
                Manager.topSSM = Manager.realTop(ConstSQL.TOP_SO_SU_MENH, con);
                Manager.topSanBoss = Manager.realTop(ConstSQL.TOP_SB, con);
...
//                Manager.topDuaNap = Manager.realTop(ConstSQL.TOP_DUA_NAP, con);
    ...
    public static void showListTopPower(Player player) {
        ...
        try {
            ...
            for (int i = 0; i < 100; i++) {
                ...
                if (player.isAdmin()) {
                    continue; // bỏ qua admin, không cho vào BXH
                }
    ...
    public static String getTopNap() {
        ...
        try (Connection conn = DBConnecter.getConnectionServer(); PreparedStatement ps = conn.prepareStatement(ConstSQL.TOP_DUA_NAP)) {
            ...
            while (rs.next()) {
                ...
                int danap = rs.getInt("danap");
                // Format số tiền với dấu ,
                String formattedDanap = String.format("%,d", danap);
                sb.append(i).append(". ").append(name)
                        .append(": ").append(formattedDanap).append(" Đã Nạp\n");
                i++;
            }
    ...
    public static String getTopSM() {
        StringBuilder sb = new StringBuilder();
        try (Connection conn = DBConnecter.getConnectionServer(); PreparedStatement ps = conn.prepareStatement(ConstSQL.TOP_DUA_SM)) {
            ...
            while (rs.next()) {
                ...
                int sm = rs.getInt("sm");
                sb.append(i).append(". ").append(name)
                        .append(": ").append(sm).append(" Sức Mạnh\n");
                i++;
            }
    ...
    public static void showListTop(Player player, int select) {
        ...
        switch (select) {
            ...
            case 6 ->
                tops = Manager.topPhaoBong;
            case 7 ->
                tops = Manager.topLixi;
            case 8 ->
                tops = Manager.topSanBoss;
            case 9 ->
                tops = Manager.topSSM;
        ...
        try {
            ...
            for (int i = 0; i < tops.size(); i++) {
                ...
                switch (select) {
                    ...
                    case 6 -> {
                        msg.writer().writeUTF("Đang đã đốt " + top.getPhaobong() + " pháo bông");
                        msg.writer().writeUTF(getTimeLeft(top.getLasttime()));
                    }
                    case 7 -> {
                        msg.writer().writeUTF("Đã mở " + top.getLixi() + " lì xì");
                        msg.writer().writeUTF(getTimeLeft(top.getLasttime()));
                    }
                    case 8 -> {
                        msg.writer().writeUTF("Đã kill " + top.getPointsBoss() + " con boss");
                        msg.writer().writeUTF(getTimeLeft(top.getLasttime()));
                    }
                    case 9 -> {
                        msg.writer().writeUTF("Điểm SSM: " + top.getCash());
                        msg.writer().writeUTF("" + top.getCash() + " Điểm");
                    }
