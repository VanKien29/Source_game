...
//    }
    ...
    public void regisAccount(Session session, Message _msg) {
        try {
            ...
            if (rs.next()) {
                ...
                int affected = DBConnecter.executeUpdate(
                        "INSERT INTO account (username, password, admin, cash, danap, DiemDanh) VALUES (?, ?, ?, ?, ?, ?)",
                        user, pass, 0, 0, 0, 0
                );
    ...
    public void Send_Info_NV(Player pl) {
        ...
        try {
            ...
            msg.writer().writeInt((int) pl.id);
            msg.writeLongByHoandz(Util.maxIntValue(pl.nPoint.hp), cn.readInt);
            msg.writer().writeByte(0);// Hiệu ứng Ăn Đậu
            msg.writeLongByHoandz(Util.maxIntValue(pl.nPoint.hpMax), cn.readInt);
    ...
    public void point(Player player) {
        ...
        if (!player.isPet && !player.isBoss && !player.isNewPet) {
            ...
            try {
                msg = new Message(-42);
                msg.writeLongByHoandz(Util.maxIntValue(player.nPoint.hpg), cn.readInt);
                msg.writeLongByHoandz(Util.maxIntValue(player.nPoint.mpg), cn.readInt);
                msg.writeLongByHoandz(Util.maxIntValue(player.nPoint.dameg), cn.readInt);
                msg.writeLongByHoandz(Util.maxIntValue(player.nPoint.hpMax), cn.readInt);// hp full
                msg.writeLongByHoandz(Util.maxIntValue(player.nPoint.mpMax), cn.readInt);// mp full
                msg.writeLongByHoandz(Util.maxIntValue(player.nPoint.hp), cn.readInt);// hp
                msg.writeLongByHoandz(Util.maxIntValue(player.nPoint.mp), cn.readInt);// mp
                msg.writer().writeByte(player.nPoint.speed);// speed
                msg.writer().writeByte(20);
                msg.writer().writeByte(20);
                msg.writer().writeByte(1);
                msg.writeLongByHoandz(Util.maxIntValue(player.nPoint.dame), cn.readInt);// dam base
    ...
    public String getCurrStrLevel(Player pl) {
        ...
        int idx = getCurrLevel(pl);

        if (captions == null || captions.isEmpty()) {
            return "Chưa có cấp độ"; // fallback
        }

        if (idx < 0) {
            idx = 0;
        }
        if (idx >= captions.size()) {
            idx = captions.size() - 1;
        }
    ...
    public int getCurrLevel(Player pl) {
        ...
        if (sucmanh < 3000) {
            return 0;
        } else if (sucmanh < 15_000) {
            return 1;
        } else if (sucmanh < 40_000) {
            return 2;
        } else if (sucmanh < 90_000) {
            return 3;
        } else if (sucmanh < 170_000) {
            return 4;
        } else if (sucmanh < 340_000) {
            return 5;
        } else if (sucmanh < 700_000) {
            return 6;
        } else if (sucmanh < 1_500_000) {
            return 7;
        } else if (sucmanh < 15_000_000) {
            return 8;
        } else if (sucmanh < 150_000_000) {
            return 9;
        } else if (sucmanh < 1_500_000_000) {
            return 10;
        } else if (sucmanh < 5_000_000_000L) {
            return 11;
        } else if (sucmanh < 10_000_000_000L) {
            return 12;
        } else if (sucmanh < 40_000_000_000L) {
            return 13;
        } else if (sucmanh < 50_010_000_000L) {
            return 14;
        } else if (sucmanh < 60_010_000_000L) {
            return 15;
        } else if (sucmanh < 70_010_000_000L) {
            return 16;
        } else if (sucmanh < 80_010_000_000L) {
            return 17;
        } else if (sucmanh < 100_010_000_000L) {
            return 18;
        } else if (sucmanh < 120_010_000_000L) {
            return 19;
        } else if (sucmanh < 120_010_000_000L) {
            return 20;
        } else if (sucmanh < 150_010_000_000L) {
            return 21;
        } else if (sucmanh < 200_010_000_000L) {
            return 22;
        } else if (sucmanh < 520_010_000_000L) {
            return 23;
        }
        return 24;
    ...
    public void hsChar(Player pl, long hp, long mp) {
        ...
        try {
            ...
            msg.writeLongByHoandz(Util.maxIntValue(mp), cn.readInt);
    ...
    public void sendLogout(Player player) {
        for (int i = 5; i >= 0; i--) {
            Service.gI().sendThongBao(player, "Tự động đăng xuất sau " + i + " giây");
            try {
                Thread.sleep(1000);
            } catch (Exception e) {
            }
    ...
    public void sendThongBao(Player pl, String thongBao) {
        ...
        try {
            Message msg = new Message(-25);
            msg.writer().writeUTF(thongBao);
            pl.sendMessage(msg);
            msg.cleanup(); // Đảm bảo giải phóng tài nguyên
    ...
    public void sendChiSoPetGoc(Player pl) {
        ...
        try {
            ...
            msg.writeLongByHoandz(Util.maxIntValue(pl.pet.nPoint.hpg), cn.readInt);
            msg.writeLongByHoandz(Util.maxIntValue(pl.pet.nPoint.mpg), cn.readInt);
            msg.writeLongByHoandz(Util.maxIntValue(pl.pet.nPoint.dameg), cn.readInt);
...
//                    msg.writer().writeInt(pl.pet.nPoint.critg);
//                }
                msg.writeLongByHoandz(Util.maxIntValue(pl.pet.nPoint.hp), cn.readInt); //hp
                msg.writeLongByHoandz(Util.maxIntValue(pl.pet.nPoint.hpMax), cn.readInt); //hpfull
                msg.writeLongByHoandz(Util.maxIntValue(pl.pet.nPoint.mp), cn.readInt); //mp
                msg.writeLongByHoandz(Util.maxIntValue(pl.pet.nPoint.mpMax), cn.readInt); //mpfull
                msg.writeLongByHoandz(Util.maxIntValue(pl.pet.nPoint.dame), cn.readInt); //damefull
    ...
    public void setClientType(MySession session, Message msg) {
        try {
            ...
            // if (key.equals("Hoandz") && code == 31072002 &&
            // token.equals("Hoandz")) {
    ...
    public void sendMabuAttackSkill(Player player) {
        ...
        try {
            ...
            for (Player plM : player.zone.getNotBosses()) {
                ...
                msg.writeLongByHoandz(Util.maxIntValue(damage), cn.readInt);
            }
            sendMessAllPlayerInMap(player, msg);
            msg.cleanup();
        } catch (Exception e) {
