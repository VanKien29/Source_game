...
public class MapService {
    ...
    public WayPoint getWaypointPlayerIn(Player player) {
        Optional<WayPoint> result = player.zone.map.wayPoints.parallelStream()
                .filter(wp -> isPlayerInWaypoint(player, wp))
                .findFirst();

        return result.orElse(null);
    }

    private boolean isPlayerInWaypoint(Player player, WayPoint wp) {
        return player.location.x >= wp.minX && player.location.x <= wp.maxX
    ...
    public Map getMapForKyNgo() {
        int[] listmap = {0, 1, 2, 3, 4, 5, 6, 27, 28, 29, 30, 42};
    ...
    public boolean isMapCalick(int mapId) {
        return mapId >=27 && mapId <= 29;
    }
    ...
    public void sendPlayerMove(Player player) {
        ...
        try {
            ...
            System.out.println("Lỗi sendPlayerMove");
        }
    }
    ...
    public boolean isMapRungCay(int mapId) {
        return mapId == 177;
    }
}
