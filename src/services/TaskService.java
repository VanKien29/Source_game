...
public class TaskService {
    ...
    public void checkSoSuMenhKillMob(Player player, Mob mob) {
        ...
        switch (mob.tempId) {
            ...
            case ConstMob.NAPPA:
                SoSuMenhService.gI().updateProgress(player, 1, 1);
                break;

            case ConstMob.SOLDIER:
                SoSuMenhService.gI().updateProgress(player, 2, 1);
                break;

            case ConstMob.APPULE:
                SoSuMenhService.gI().updateProgress(player, 3, 1);
                break;

            case ConstMob.KHI_LONG_DO:
                SoSuMenhService.gI().updateProgress(player, 4, 1);
                break;

            case ConstMob.KHI_LONG_DEN:
                SoSuMenhService.gI().updateProgress(player, 5, 1);
                break;

            case ConstMob.KHI_LONG_VANG:
                SoSuMenhService.gI().updateProgress(player, 6, 1);
                break;

            case ConstMob.XEN_CON_CAP_1:
                SoSuMenhService.gI().updateProgress(player, 17, 1);
                break;

            case ConstMob.XEN_CON_CAP_3:
                SoSuMenhService.gI().updateProgress(player, 18, 1);
                break;

            case ConstMob.XEN_CON_CAP_5:
                SoSuMenhService.gI().updateProgress(player, 19, 1);
                break;

            case ConstMob.XEN_CON_CAP_8:
                SoSuMenhService.gI().updateProgress(player, 20, 1);
                break;
        }
    }
        ...
        if (TaskService.gI().isCurrentTask(player, idTaskCustom)) {
            ...
            switch (idTaskCustom) {
                ...
                case ConstTask.TASK_27_5:
                    PlayerDAO.updateActiveById(player.getSession().userId);
            ...
            // ===== CHỈ UPDATE DB KHI HOÀN THÀNH =====
            if (taskJustCompleted) {
                long now = System.currentTimeMillis();
                if (Util.canDoWithTime(player.lastTimeUpdateTask, 5000)) {
                    PlayerDAO.updateLastTimeUpdateTask(player, now);
                    player.lastTimeUpdateTask = now;
                }
            }
        ...
        if (player.playerTask.sideTask.template != null) {
            if (player.playerTask.sideTask.isDone()) {
                ...
                if (InventoryService.gI().getCountEmptyBag(player) > 1) {
                    ...
                    Item thungnep = ItemService.gI().createNewItem((short) ConstItem.THUNG_NEP);
                    InventoryService.gI().addItemBag(player, thungnep);
                    InventoryService.gI().sendItemBag(player);
                    player.inventory.addGold(goldReward);
                    Service.gI().sendMoney(player);
                    Service.gI().sendThongBao(player, "Bạn nhận được "
                            + Util.numberToMoney(goldReward) + " vàng và " + thungnep.template.name);
                    player.playerTask.sideTask.reset();
                } else {
