...
public class EventDAO {
    ...
    public static void loadInternationalWomensDayEvent() {
        try (Connection con = DBConnecter.getConnectionServer();) {
            PreparedStatement ps = con
                    .prepareStatement("SELECT `data` FROM `event` WHERE `name` = 'international_womens_day'");
    ...
    public static void save() {
        try {
            ...
            DBConnecter.executeUpdate("UPDATE `event` SET `data` = ? WHERE `name` = 'international_womens_day'",
                    jsonData);
        } catch (Exception e) {
