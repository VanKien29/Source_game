...
public class NDVSqlFetcher {
    ...
    public static Player login(MySession session, AntiLogin al) {
        ...
        try {
            ...
            if (rs == null || !rs.next()) {
                al.wrong();
                Service.gI().sendThongBaoOK(session, al.getNotifyWrongCount());
            ...
            if (rs.first()) {
                ...
                session.userId = accountId;
                Client.gI().tryLogin(accountId, session);
                if ((plInGame = Client.gI().getPlayerByUser(accountId)) != null) {
                    if (plInGame.getSession() != null) {
                        Client.gI().kickSession(plInGame.getSession());
                        Client.gI().removeLogin(accountId);
                    }
                }
                ...
                session.coin = rs.getInt("coin");
                session.diemdanh = rs.getInt("DiemDanh");
                session.danap = rs.getInt("danap");
                session.diemReceive = rs.getInt("diem_da_nhan");
//                session.vip = rs.getInt("vip");
//                session.hasReceivedVIP = rs.getBoolean("hasReceivedVIP");
                session.receivedVIP1 = rs.getBoolean("receivedVIP1");
                session.receivedVIP2 = rs.getBoolean("receivedVIP2");
                session.receivedVIP3 = rs.getBoolean("receivedVIP3");
//                session.lastTimeReceivedVIP = rs.getLong("lastTimeReceivedVIP");
                ...
                } else if (Manager.DEBUG && !session.isAdmin) {
                    Service.gI().sendThongBaoOK(session, "Sever đang được ADMIN xử lý, vui lòng quay lại sau !");
    ...
    private static Player loadPlayer(NDVResultSet rs, boolean isOffline) throws Exception {
        ...
        try {
            ...
            int dh1 = Integer.parseInt(String.valueOf(dataArray.size() > 0 ? dataArray.get(0) : 0));
            int dh2 = Integer.parseInt(String.valueOf(dataArray.size() > 1 ? dataArray.get(1) : 0));
            int dh3 = Integer.parseInt(String.valueOf(dataArray.size() > 2 ? dataArray.get(2) : 0));
            int dh4 = Integer.parseInt(String.valueOf(dataArray.size() > 3 ? dataArray.get(3) : 0));
            int dh5 = Integer.parseInt(String.valueOf(dataArray.size() > 4 ? dataArray.get(4) : 0));
            int dh6 = Integer.parseInt(String.valueOf(dataArray.size() > 5 ? dataArray.get(5) : 0));
            int dh7 = Integer.parseInt(String.valueOf(dataArray.size() > 6 ? dataArray.get(6) : 0));
            int dh8 = Integer.parseInt(String.valueOf(dataArray.size() > 7 ? dataArray.get(7) : 0));
            int dh9 = Integer.parseInt(String.valueOf(dataArray.size() > 8 ? dataArray.get(8) : 0));
            int dh10 = Integer.parseInt(String.valueOf(dataArray.size() > 9 ? dataArray.get(9) : 0));
            int dh11 = Integer.parseInt(String.valueOf(dataArray.size() > 10 ? dataArray.get(10) : 0));

            player.effect.setPointDaiGiaMoiNhu(dh1);
            player.effect.setPointOngThanVeChai(dh2);
            player.effect.setPointBiMocSachTui(dh3);
            player.effect.setPointPhanCung(dh4);
            player.effect.setPointGoDauTre(dh5);
            player.effect.setPointGoDauTre1(dh6);
            player.effect.setPointGoDauTre2(dh7);
            player.effect.setPointXMas(dh8);
            player.effect.setPointEmDepEmXinh(dh9);
            player.effect.setPointAnBamTraXanh(dh10);
            player.effect.setPointTayNhanhHonNao(dh11);
            dataArray.clear();
...
// ====== CÓ DATA SỔ SỨ MỆNH ======
            if (jsonobject != null && !jsonobject.isEmpty()) {
                ...
                // ===== BASIC INFO =====
                player.sosumenhplayer.setLevel(
                        Integer.parseInt(String.valueOf(jsonobject.get("level")))
                );
                player.sosumenhplayer.setPoint(
                        Integer.parseInt(String.valueOf(jsonobject.get("point")))
                );
                player.sosumenhplayer.setVip(
                        Boolean.parseBoolean(String.valueOf(jsonobject.get("vip")))
                );
                ...
                if (jA != null) {
                    for (int i = 0; i < jA.size() && i < 20; i++) {
                        if (jA.get(i) != null) {
                            player.sosumenhplayer.reward[i]
                                    = Boolean.parseBoolean(jA.get(i).toString());
                        }
                    }
                }
                ...
                if (jA != null) {
                    for (int i = 0; i < jA.size() && i < 20; i++) {
                        if (jA.get(i) != null) {
                            player.sosumenhplayer.rewardVip[i]
                                    = Boolean.parseBoolean(jA.get(i).toString());
                        }
                    }
                }

                // ===== LOAD TASK MAIN TỪ DB =====
                ...
                if (jA != null) {
                    for (int i = 0; i < jA.size(); i++) {
                        ...
                        if (taskArray == null || taskArray.size() < 3) {
                            continue;
                        }
                        ...
                        // maxCount LUÔN LẤY TỪ TEMPLATE
                        SoSuMenhTaskTemplate template
                                = SoSuMenhManager.getInstance().findById(smm.idTask);
                        smm.maxCount = template != null ? template.getMaxCount() : 0;

                        player.sosumenhplayer.ssmTaskMain.add(smm);
                    }
                }

                // ===== SYNC NHIỆM VỤ MỚI (KHÔNG RESET) =====
                for (SoSuMenhTaskTemplate template : SoSuMenhManager.getInstance().list) {
                    boolean found = false;
                    for (SoSuMenhTaskMain t : player.sosumenhplayer.ssmTaskMain) {
                        if (t.idTask == template.getId()) {
                            found = true;
                            break;
                        }
                    }
                    if (!found) {
                        ...
                        smm.countTask = 0;
                        smm.finish = false;
                        smm.maxCount = template.getMaxCount();
                        player.sosumenhplayer.ssmTaskMain.add(smm);
                    }
                }

                // clear tạm (không bắt buộc)
                if (jA != null) {
                    jA.clear();
                }
                jsonobject.clear();
            } // ====== PLAYER HOÀN TOÀN MỚI (CHƯA CÓ DATA) ======
            else {
                player.sosumenhplayer.setLevel(0);
                player.sosumenhplayer.setPoint(0);
                player.sosumenhplayer.setVip(false);
            ...
            player.event.setEventPointLixi(rs.getInt("lixi"));
            ...
            player.lastTimeUpdateTask = rs.getLong("lastTimeUpdateTask");
            ...
            int maxSlots = Inventory.BODY_SLOT_COUNT;
            int sizeFromDb = (dataArray != null ? dataArray.size() : 0);
...
// ====== HẾT DATA BODY ======
            ...
            if (dataArray.size() > 29) {
                timehoadao = Integer.parseInt(String.valueOf(dataArray.get(29)));
            }
            if (dataArray.size() > 30) {
                timehoamai = Integer.parseInt(String.valueOf(dataArray.get(30)));
            }
            if (dataArray.size() > 31) {
                timebanhtet = Integer.parseInt(String.valueOf(dataArray.get(31)));
            }
            if (dataArray.size() > 32) {
                timebanhchung = Integer.parseInt(String.valueOf(dataArray.get(32)));
            }
            if (dataArray.size() > 33) {
                timeKhongLo = Integer.parseInt(dataArray.get(33).toString());
            }
            if (dataArray.size() > 34) {
                timeThom = Integer.parseInt(dataArray.get(34).toString());
            }
            if (dataArray.size() > 35) {
                timeSauRieng = Integer.parseInt(dataArray.get(35).toString());
            }
            if (dataArray.size() > 36) {
                timePhoTaiHanh = Integer.parseInt(String.valueOf(dataArray.get(36)));
            }
            if (dataArray.size() > 37) {
                timePhoNam = Integer.parseInt(String.valueOf(dataArray.get(37)));
            }
            if (dataArray.size() > 38) {
                timePhoTaiBo = Integer.parseInt(String.valueOf(dataArray.get(38)));
            }
            ...
            player.itemTime.lastTimeXimuoihoadao = System.currentTimeMillis() - (ItemTime.TIME_ITEM - timehoadao);
            player.itemTime.lastTimeXimuoihoamai = System.currentTimeMillis() - (ItemTime.TIME_ITEM - timehoamai);
            player.itemTime.lastTimeBanhtet = System.currentTimeMillis() - (ItemTime.TIME_CMS - timebanhtet);
            player.itemTime.lastTimeBanhchung = System.currentTimeMillis() - (ItemTime.TIME_CMS - timebanhchung);
            player.itemTime.lastTimeKhongLo = System.currentTimeMillis() - (ItemTime.TIME_ITEM - timeKhongLo);
            player.itemTime.lastTimeThom = System.currentTimeMillis() - (ItemTime.TIME_ITEM - timeThom);
            player.itemTime.lastTimeSauRieng = System.currentTimeMillis() - (ItemTime.TIME_ITEM - timeSauRieng);
            player.itemTime.lastTimePhoTaiHanh = System.currentTimeMillis() - (ItemTime.TIME_ITEM - timePhoTaiHanh);
            player.itemTime.lastTimePhoNam = System.currentTimeMillis() - (ItemTime.TIME_ITEM - timePhoNam);
            player.itemTime.lastTimePhoTaiBo = System.currentTimeMillis() - (ItemTime.TIME_ITEM - timePhoTaiBo);
            ...
            player.itemTime.isXimuoihoadao = timehoadao != 0;
            player.itemTime.isXimuoihoamai = timehoamai != 0;
            player.itemTime.isBanhtet = timebanhtet != 0;
            player.itemTime.isBanhchung = timebanhchung != 0;
            player.itemTime.nuocMiaKhongLo = timeKhongLo != 0;
            player.itemTime.nuocMiaThom = timeThom != 0;
            player.itemTime.nuocMiaSauRieng = timeSauRieng != 0;
            player.itemTime.isUsePhoTaiHanh = timePhoTaiHanh != 0;
            player.itemTime.isUsePhoNam = timePhoNam != 0;
            player.itemTime.isUsePhoTaiBo = timePhoTaiBo != 0;
            ...
            dataArray = (JSONArray) JSONValue.parse(rs.getString("nangcap"));
            player.cap = Integer.parseInt(String.valueOf(dataArray.get(0)));
            player.level = Integer.parseInt(String.valueOf(dataArray.get(1)));
            dataArray.clear();
            // ====== DATA SỐ Ô TRANG BỊ MỞ KHÓA ======
            try {
                dataArray = (JSONArray) JSONValue.parse(rs.getString("data_body_slot"));
                if (dataArray != null && !dataArray.isEmpty()) {
                    player.bodySlotExtra = Integer.parseInt(String.valueOf(dataArray.get(0)));
                } else {
                    player.bodySlotExtra = 0;
                }
            } catch (Exception ex) {
                player.bodySlotExtra = 0;
            }
            dataArray.clear();
            try {
                player.tienCapLevel = (byte) rs.getInt("tien_cap_level");
                player.tienCapGold = rs.getInt("tien_cap_gold");
                player.tienCapKill = rs.getInt("tien_cap_kill");
                player.tienCapKillBoss = rs.getInt("tien_cap_killboss");
            } catch (Exception e) {
                player.tienCapLevel = 0;
            ...
            if (!petData.isEmpty()) {
                ...
                // data body
                JSONArray petBodyDb = (JSONArray) JSONValue.parse(String.valueOf(petData.get(2)));
                ...
                if (pet.typePet == 1) {
                    ...
                    slotByType = 11;
                } else if (pet.typePet == 8) {
                    slotByType = 12;
                } else if (pet.typePet == 9) {
                    slotByType = 13;
                }

                // số slot thực tế đang được lưu trong DB
                sizeFromDb = (petBodyDb != null ? petBodyDb.size() : 0);

                // quan trọng: không bao giờ load ít hơn DB đã lưu
                int expectedSlots = Math.max(slotByType, sizeFromDb);

                for (int i = 0; i < expectedSlots; i++) {
                    Item item = ItemService.gI().createItemNull();

                    if (i < sizeFromDb) {
                        JSONArray dataItem = (JSONArray) JSONValue.parse(String.valueOf(petBodyDb.get(i)));
                        if (dataItem != null && dataItem.size() >= 4) {
                            ...
                            if (tempId != -1) {
                                ...
                                JSONArray options = (JSONArray) JSONValue.parse(String.valueOf(dataItem.get(2)));
                                if (options != null) {
                                    for (int j = 0; j < options.size(); j++) {
                                        JSONArray opt = (JSONArray) JSONValue.parse(String.valueOf(options.get(j)));
    ...
    public static boolean updateMailBox(Player player) {
        try {
            ...
            dataArray.clear();
            try (Connection con = DBConnecter.getConnectionServer(); PreparedStatement ps = con
