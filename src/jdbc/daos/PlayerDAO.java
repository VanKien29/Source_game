...
public class PlayerDAO {
    ...
    private static String createEmptyBodySlot() {
    ...
    private static String trimOrExpandItemsBody(String itemsBodyJson, int needSlots) {
        if (needSlots < 6) {
            needSlots = 6;
        }
        if (needSlots > Inventory.BODY_SLOT_COUNT) {
            needSlots = Inventory.BODY_SLOT_COUNT;
        }
        JSONArray arr = (JSONArray) JSONValue.parse(itemsBodyJson);
        if (arr == null) {
            arr = new JSONArray();
        }
        while (arr.size() > needSlots) {
            arr.remove(arr.size() - 1);
        }
        while (arr.size() < needSlots) {
            arr.add(createEmptyBodySlot());
        }
    ...
    public static boolean createNewPlayer(int userId, String name, byte gender, int hair) {
        try {
            ...
            DBConnecter.executeUpdate("insert into player"
                    ...
                    + "items_bag, items_box, items_box_collection, items_box_lucky_round, item_mails_box, items_daban, friends, enemies, "
                    + "data_intrinsic, data_item_time, data_task, data_mabu_egg, data_charm, skills, "
                    + "skills_shortcut, pet, data_black_ball, data_side_task, data_kol_task, "
                    + "dailyGift, Achievement, Achievement_SucManh, phaobong, lixi, nangcap, data_body_slot, data_danh_hieu) "
                    + "values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                    userId, name, hair, gender, 0, -1,
                    inventory, location, point, magicTree, itemsBody,
                    itemsBag, itemsBox, itemsBoxCollection, itemsBoxLuckyRound, itemsMailBox, itemsDaBan,
                    friends, enemies, intrinsic, itemTime, task, mabuEgg, charms, skills,
                    skillsShortcut, petData, dataBlackBall, dataSideTask, koltask,
                    dailyGift, Archievement, ArchievementSucManh, 0, 0, nangcap, dataBodySlot,dataDanhhieu
            );
    ...
    public static void updatePlayer(Player player) {
        if (player != null && player.iDMark != null && player.iDMark.isLoadedAllDataPlayer()) {
            ...
            try {
                ...
                // data body
                int needBodySlots = 6 + player.bodySlotExtra; // bodySlotExtra bạn đã có
                if (needBodySlots > Inventory.BODY_SLOT_COUNT) {
                    needBodySlots = Inventory.BODY_SLOT_COUNT;
                }
                if (needBodySlots < 6) {
                    needBodySlots = 6;
                }
                ...
                // chỉ lưu đúng số slot được mở
                for (int i = 0; i < needBodySlots; i++) {
                ...
                // phòng trường hợp data đang lệch (old data 13 slot hoặc thiếu slot)
                itemsBody = trimOrExpandItemsBody(itemsBody, needBodySlots);
                ...
                dataArray.add((player.itemTime.isXimuoihoadao
                        ? (ItemTime.TIME_ITEM - (System.currentTimeMillis() - player.itemTime.lastTimeXimuoihoadao))
                        : 0));
                dataArray.add((player.itemTime.isXimuoihoamai
                        ? (ItemTime.TIME_ITEM - (System.currentTimeMillis() - player.itemTime.lastTimeXimuoihoamai))
                        : 0));
                dataArray.add((player.itemTime.isBanhtet
                        ? (ItemTime.TIME_CMS - (System.currentTimeMillis() - player.itemTime.lastTimeBanhtet))
                        : 0));
                dataArray.add((player.itemTime.isBanhchung
                        ? (ItemTime.TIME_CMS - (System.currentTimeMillis() - player.itemTime.lastTimeBanhchung))
                        : 0));
                dataArray.add(player.itemTime.nuocMiaKhongLo
                        ? (ItemTime.TIME_ITEM - (System.currentTimeMillis() - player.itemTime.lastTimeKhongLo))
                        : 0);
                dataArray.add(player.itemTime.nuocMiaThom
                        ? (ItemTime.TIME_ITEM - (System.currentTimeMillis() - player.itemTime.lastTimeThom))
                        : 0);
                dataArray.add(player.itemTime.nuocMiaSauRieng
                        ? (ItemTime.TIME_ITEM - (System.currentTimeMillis() - player.itemTime.lastTimeSauRieng))
                        : 0);
                dataArray.add((player.itemTime.isUsePhoTaiHanh
                        ? (ItemTime.TIME_ITEM - (System.currentTimeMillis() - player.itemTime.lastTimePhoTaiHanh))
                        : 0));
                dataArray.add((player.itemTime.isUsePhoNam
                        ? (ItemTime.TIME_ITEM - (System.currentTimeMillis() - player.itemTime.lastTimePhoNam))
                        : 0));
                dataArray.add((player.itemTime.isUsePhoTaiBo
                        ? (ItemTime.TIME_ITEM - (System.currentTimeMillis() - player.itemTime.lastTimePhoTaiBo))
                        : 0));
                ...
                dataArray.add(player.cap);
                dataArray.add(player.level);
                String datanangcap = dataArray.toJSONString();
                dataArray.clear();

                dataArray.add(player.bodySlotExtra);
                String dataBodySlot = dataArray.toJSONString();
                dataArray.clear();
...
//                json.put("coutday", player.sosumenhplayer.getCoutday());
                ...
                for (SoSuMenhTaskMain ssmTask : player.sosumenhplayer.ssmTaskMain) {
                    ...
                    taskArray.add(ssmTask.maxCount);
                ...
                dataShopDanhHieu.add(player.effect.getPointGoDauTre());
                dataShopDanhHieu.add(player.effect.getPointGoDauTre1());
                dataShopDanhHieu.add(player.effect.getPointGoDauTre2());
                dataShopDanhHieu.add(player.effect.getPointXMas());
                ...
                String query = "update player set head = ?, have_tennis_space_ship = ?, "
                        ...
                        + "items_body = ?, items_bag = ?, items_box = ?, items_box_collection= ?, items_box_lucky_round = ?, item_mails_box =?, items_daban = ?, friends = ?, "
                        ...
                        + "rank = ?, data_super_rank = ?, data_achievement = ?, giftcode = ?, firstTimeLogin = ?, dailyGift = ?, "
                        + "event_point = ?, event_point_boss = ?, event_point_nhs = ?, event_point_quai = ?, diem_quy_lao = ?, "
                        + "hp_point_fusion = ?, mp_point_fusion = ?, dame_point_fusion = ?, phaobong = ?, lixi = ?,"
                        + " hpbang =?, mpbang=?, damebang =?, critbang =?, so_su_menh = ?, nangcap = ?,data_body_slot = ?, "
                        + "tien_cap_level = ?, tien_cap_gold = ?, tien_cap_kill = ?, tien_cap_killboss = ? , data_danh_hieu = ?"
                        + " where id = ?";
                ...
                DBConnecter.executeUpdate(query,
                        ...
                        player.event.getEventPointLixi(),
    ...
    public static void updateActiveById(int id) {
        ...
        try (Connection con = DBConnecter.getConnectionServer(); PreparedStatement ps = con.prepareStatement(sql)) {
            ...
            Logger.logException(PlayerDAO.class, e, "Lỗi updateActiveById");
        }
    }

    public static boolean addcash(String name, int num) {
        String updateQuery = "UPDATE account a JOIN player p ON a.id = p.account_id SET a.cash = a.cash + ?, a.danap = a.danap + ? WHERE p.name = ?";
        try (Connection con = DBConnecter.getConnectionServer(); PreparedStatement ps = con.prepareStatement(updateQuery)) {
            ps.setInt(1, num);
            ps.setInt(2, num);
            ps.setString(3, name);
            ...
            Logger.logException(PlayerDAO.class, e, "Loi o ham cash ");
            return false;
        }
    }
    ...
    public static boolean subGoldBarUnLock(Player player, int num) {
        String updateQuery = "UPDATE account SET tv_mo = tv_mo - ? WHERE id = ?";
        try (Connection con = DBConnecter.getConnectionServer(); PreparedStatement ps = con.prepareStatement(updateQuery)) {
            ...
            int rowsAffected = ps.executeUpdate();
            if (rowsAffected > 0) {
                player.getSession().goldBarUnLock -= num;
                return true;
            } else {
                return false;
            }
            ...
            Logger.logException(PlayerDAO.class, e,
                    "Error updating goldBarUnLock for player " + player.name);
            return false;
        }
    }
    ...
    public static void subVIP(Player player, int vipLevel) {
        try (Connection con = DBConnecter.getConnectionServer()) {
            // Chọn cột tương ứng với VIP đã mua
            String vipColumn = "receivedVIP" + vipLevel;

            // Kiểm tra nếu player đã nhận VIP này rồi thì không làm gì
            if (player.getSession().hasReceivedVIPLevel(vipLevel)) {
                return;
            }

            // Cập nhật bảng account chỉ phần receivedVIP
            String sql = "UPDATE account SET " + vipColumn + " = 1 WHERE id = ?";
            try (PreparedStatement ps = con.prepareStatement(sql)) {
                ps.setInt(1, player.getSession().userId);
                ps.executeUpdate();

                // Cập nhật trạng thái trong session
                player.getSession().setReceivedVIPLevel(vipLevel, true);
            }
        } catch (SQLException e) {
            Logger.logException(PlayerDAO.class, e, "Lỗi update receivedVIP" + vipLevel + " " + player.name);
        }
    }
    ...
    public static boolean createPlAo(int userId, String name, byte gender, int hair) {
        try {
            ...
            dataArray.clear();
            DBConnecter.executeUpdate("insert into player"
                    + "(id, account_id, name, head, gender, have_tennis_space_ship, clan_id, "
                    + "data_inventory, data_location, data_point, data_magic_tree, items_body, "
                    ...
                    + "data_black_ball, data_side_task, Achievement, data_danh_hieu) "
    ...
    public static boolean addPointEvent(Player player, int num) {
        try (Connection con = DBConnecter.getConnectionServer(); PreparedStatement ps = con.prepareStatement("update account set diem_da_nhan = (diem_da_nhan + ?) where id = ?")) {
            ps.setInt(1, num);
            ps.setInt(2, player.getSession().userId);
            ps.executeUpdate();
