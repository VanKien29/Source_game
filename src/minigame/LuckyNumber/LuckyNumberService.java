...
public class LuckyNumberService {
    ...
    public static void addNumber(Player pl, int number) {
        if (!LuckyNumber.players.stream().filter(d -> d.id == pl.id && d.id == pl.id && d.number == number).toList()
                .isEmpty()) {
        ...
        if (pl.iDMark.isGemCSMM()) {
            if (pl.inventory.gem < LuckyNumberCost.costPlayGem) {
                Service.gI().sendThongBao(pl, "Bạn không đủ ngọc, còn thiếu "
                        + (LuckyNumberCost.costPlayGem - pl.inventory.gem) + " ngọc nữa");
                return;
            }
            ...
            if (pl.inventory.gold < LuckyNumberCost.costPlayGold) {
                Service.gI().sendThongBao(pl, "Bạn không đủ vàng, còn thiếu "
                        + Util.numberToMoney(LuckyNumberCost.costGold - pl.inventory.gold) + " vàng nữa");
                return;
            }
