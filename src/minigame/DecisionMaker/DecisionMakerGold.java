...
public class DecisionMakerGold {
    ...
    public static void showMenuSelect(Npc npc, Player player) {
        ...
        npc.createOtherMenu(player, LyTieuNuong.ConstMiniGame.MENU_PLAY_DECISION_MAKER_GOLD,
                "Tổng giải thưởng: " + Util.mumberToLouis(totalNormal) + " vàng, cơ hội trúng của bạn là: "
                        + DecisionMakerService.getPercent(player, DecisionMakerCost.VANG, true) + "%\n"
                        + "Tổng giải VIP: " + Util.mumberToLouis(totalVIP) + " vàng, cơ hội trúng của bạn là: "
                        + DecisionMakerService.getPercent(player, DecisionMakerCost.VANG, false) + "%\n"
    ...
    public static void selectPlay(Npc npc, Player player, boolean isNormal) {
        ...
        if (player.inventory.gold < money) {
            Service.gI().sendThongBao(player,
                    "Bạn không đủ vàng, còn thiếu " + (money - player.inventory.gold) + " vàng nữa");
            return;
        }
