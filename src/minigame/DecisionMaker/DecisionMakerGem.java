    ...
    public static void showMenuSelect(Npc npc, Player player) {
        ...
        npc.createOtherMenu(player, LyTieuNuong.ConstMiniGame.MENU_PLAY_DECISION_MAKER_GEM,
                "Tổng giải thưởng: " + Util.mumberToLouis(totalNormal) + " hồng ngọc, cơ hội trúng của bạn là: "
                        + DecisionMakerService.getPercent(player, DecisionMakerCost.NGOC_XANH, true) + "%\n"
                        + "Tổng giải VIP: " + Util.mumberToLouis(totalVIP) + " hồng ngọc, cơ hội trúng của bạn là: "
                        + DecisionMakerService.getPercent(player, DecisionMakerCost.NGOC_XANH, false) + "%\n"
    ...
    public static void selectPlay(Npc npc, Player player, boolean isNormal) {
        ...
        if (player.inventory.gem < money) {
            Service.gI().sendThongBao(player,
                    "Bạn không đủ ngọc, còn thiếu " + (money - player.inventory.gem) + " ngọc nữa");
            return;
        }
