...
public class DecisionMakerRuby {
    public static void showMenuSelect(Npc npc, Player player) {
        ...
        npc.createOtherMenu(player, LyTieuNuong.ConstMiniGame.MENU_PLAY_DECISION_MAKER_RUBY,
                "Tổng giải thưởng: " + Util.mumberToLouis(totalNormal) + " hồng ngọc, cơ hội trúng của bạn là: "
                        + DecisionMakerService.getPercent(player, DecisionMakerCost.HONG_NGOC, true) + "%\n"
                        + "Tổng giải VIP: " + Util.mumberToLouis(totalVIP) + " hồng ngọc, cơ hội trúng của bạn là: "
                        + DecisionMakerService.getPercent(player, DecisionMakerCost.HONG_NGOC, false) + "%\n"
    ...
    public static void selectPlay(Npc npc, Player player, boolean isNormal) {
        ...
        if (player.inventory.ruby < money) {
            Service.gI().sendThongBao(player,
                    "Bạn không đủ ngọc, còn thiếu " + (money - player.inventory.ruby) + " ngọc nữa");
            return;
        }
