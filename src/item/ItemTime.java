...
public class ItemTime {
    ...
    public static final int TIME_EAT_MEAL = 1200000;
    ...
    public boolean isXimuoihoadao;
    public boolean isXimuoihoamai;
    public long lastTimeXimuoihoadao;
    public long lastTimeXimuoihoamai;
    ...
    public boolean nuocMiaKhongLo;
    public boolean nuocMiaThom;
    public boolean nuocMiaSauRieng;
    public long lastTimeKhongLo;
    public long lastTimeThom;
    public long lastTimeSauRieng;
    public boolean isUsePhoTaiHanh;
    public boolean isUsePhoNam;
    public boolean isUsePhoTaiBo;
    public long lastTimePhoTaiHanh;
    public long lastTimePhoNam;
    public long lastTimePhoTaiBo;
    ...
    public void update() {
        ...
        if (isUsePhoTaiHanh) {
            if (Util.canDoWithTime(lastTimePhoTaiHanh, TIME_ITEM)) {
                isUsePhoTaiHanh = false;
            }
        }
        if (isUsePhoNam) {
            if (Util.canDoWithTime(lastTimePhoNam, TIME_ITEM)) {
                isUsePhoNam = false;
            }
        }
        if (isUsePhoTaiBo) {
            if (Util.canDoWithTime(lastTimePhoTaiBo, TIME_ITEM)) {
                isUsePhoTaiBo = false;
            }
        }
        if (isBanhtet) {
            if (Util.canDoWithTime(lastTimeBanhtet, TIME_CMS)) {
                isBanhtet = false;
                Service.gI().point(player);
            }
        }
        if (isBanhchung) {
            if (Util.canDoWithTime(lastTimeBanhchung, TIME_CMS)) {
                isBanhchung = false;
                Service.gI().point(player);
            }
        }
        if (isXimuoihoamai) {
            if (Util.canDoWithTime(lastTimeXimuoihoamai, TIME_ITEM)) {
                isXimuoihoamai = false;
                Service.gI().point(player);
            }
        }
        if (isXimuoihoadao) {
            if (Util.canDoWithTime(lastTimeXimuoihoadao, TIME_ITEM)) {
                isXimuoihoadao = false;
                Service.gI().point(player);
            }
        }
        if (isUseLoX2) {
