...
public class Player implements Runnable {
    ...
    public long lastTimeTXChat = 0;
    public boolean isMenuProcessing = false; // Theo dõi trạng thái đang xử lý menu
    public Timer taiXiuUpdateTimer = null;   // Timer để cập nhật menu
    public long goldTai;
    public long goldXiu;
    ...
    public int level;
    public int cap;
    public int bodySlotExtra;
    private boolean isKamiOren;
    public byte tienCapLevel;    // map với tien_cap_level
    ...
    // --------------------------------------------------------------------------
    private Set<Integer> boughtItems = new HashSet<>(); // Dùng Set để tránh trùng lặp

    // Getter và Setter cho boughtItems
    public Set<Integer> getBoughtItems() {
        return boughtItems;
    }

    public void addBoughtItem(int itemTempId) {
        boughtItems.add(itemTempId);
    }
    // --------------------------------------------------------------------------
    ...
    public void update() {
        if (!this.beforeDispose && !this.isBot) {
            try {
                if (this.zone != null || (!this.isPl() && this.zone == null)) {
                    ...
                    if (this.isPl() && this.zone != null && this.zone.map != null
                            && this.zone.map.mapId == this.gender + 21
                ...
                if ((this.zone != null && this.zone.map != null && !MapService.gI().isHome(this.zone.map.mapId))
                    ...
                    if (this.isPl() && !this.isDie() && this.effectSkill != null && !this.effectSkill.isChibi
                            ...
                            && this.zone != null && this.zone.map != null) {
                    ...
                    if (this.isPl() && !this.isDie() && this.effectSkill != null && this.effectSkill.isChibi
                        ...
                        if (this.typeChibi == 1 && this.nPoint != null) {
                        ...
                        } else if (this.typeChibi == 3 && this.nPoint != null) {
                    ...
                    if (this.isPl() && this.effectSkill != null && this.effectSkill.isMabuHold && this.nPoint != null) {
                    ...
                    if (this.zone != null && this.zone.map != null && this.effectSkin != null && this.effectSkin.xHPKI > 1
                    ...
                    if (this.zone != null && this.zone.map != null && this.effectSkin != null && this.effectSkin.xDame > 1
                            && !MapService.gI().isMapBlackBallWar(this.zone.map.mapId)) {
                    ...
                    if (this.zone != null && this.zone.map != null && this.zone.map.mapId == (21 + this.gender)) {
                        if (this.mabuEgg != null) {
                            this.mabuEgg.sendMabuEgg();
                        }
                    }

                    if (this.isPhuHoMapMabu && this.zone != null && this.zone.map != null
                            && !MapService.gI().isMapMabu2H(this.zone.map.mapId)) {
                    ...
                    if (this.isPl() && this.clan != null && this.clan.ConDuongRanDoc != null
                            ...
                            && this.talkToThanMeo && this.zone != null && this.zone.map != null
                            && this.zone.map.mapId == 47
...
//    }
    ...
    private static final short[][] idOutfitFusion = {
        ...
        {1665, 1668, 1669}, {1628, 1631, 1632}, {1638, 1636, 1637}
    };
    ...
    public byte getAura() {
        ...
        if (this.effectSkill != null && this.effectSkill.isSuper) {
            return idAuraSuper[gender][(playerSkill.getSkillbyId(gender == 0 ? 27 : gender == 1 ? 28 : 29).point - 1)
                    - numUseSkill];
        }
...
//                    return 117;
        ...
        // Ưu tiên kiểm tra thẻ có level cao nhất, nếu level bằng nhau thì chọn ID cao nhất
        Card selectedCard = null;
        int maxCardLevel = 0;
        for (Card card : this.Cards) {
            if (card != null && (card.Id == 956 || card.Id == 1204 || card.Id == 1961 || card.Id == 1962 || card.Id == 1963) && card.Level > 1) {
                if (selectedCard == null || card.Level > maxCardLevel || (card.Level == maxCardLevel && card.Id > selectedCard.Id)) {
                    maxCardLevel = card.Level;
                    selectedCard = card;
                }
            }
        }
        if (selectedCard != null) {
            final Card finalSelectedCard = selectedCard;
            RadarCard radarTemplate = RadarService.gI().RADAR_TEMPLATE.stream().filter(r -> r.Id == finalSelectedCard.Id)
    ...
    public short getHead() {
        if (this.isPl() && this.pet != null && this.fusion.typeFusion == ConstPlayer.HOP_THE_PORATA
            ...
            boolean hasItem5 = item.isNotNullItem() && (item.template.id == 1817 || item.template.id == 1818);
            boolean hasItem6 = petItem.isNotNullItem() && (petItem.template.id == 1818 || petItem.template.id == 1817);
            boolean sameItem2 = item.isNotNullItem() && petItem.isNotNullItem() && item.template.id == petItem.template.id;
        ...
        } else if (this.itemTime != null && this.itemTime.isUseKhauTrang) {
            if (this.gender == 1) {
                return 777;
    ...
    public short getMount() {
        ...
        if (item.template.type == 24) {
            ...
            if (item.template.id < 500) {
                ...
                Short mountNum = (Short) DataGame.MAP_MOUNT_NUM.get(item.template.id);
                return mountNum != null ? (short) mountNum : (short) -1;
            }
        }
    }
...
//            }
    ...
    public int getOpenedBodySlotCount() {
        int baseSlots = 6;                 
        int extra = this.bodySlotExtra;
        int maxSlots = Inventory.BODY_SLOT_COUNT; // 13

        int total = baseSlots + extra;
        if (total > maxSlots) {
            total = maxSlots;
        }
        return total;
    ...
    public boolean isBodySlotUnlocked(int index) {
        return index >= 0 && index < getOpenedBodySlotCount();
    }
}
