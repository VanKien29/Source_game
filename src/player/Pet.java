...
public class Pet extends Player {
    ...
    public void fusionKamiOren() {
        if (master.fusion.typeFusion != ConstPlayer.NON_FUSION) {
            Item item = master.inventory.itemsBody.get(5);
            Item petItem = this.inventory.itemsBody.get(5);
            boolean hasItem = item.isNotNullItem() && (item.template.id == 1817 || item.template.id == 1818);
        ...
        if (this.nPoint != null && Util.canDoWithTime(lastTimeIncreasePoint, 100)) {
            if (status != FUSION) {
                ...
                if (Util.isTrue(1, 100)) {
                    // Tăng chỉ số 3 hoặc 4 (giáp hoặc chí mạng)
                    byte type = (byte) Util.nextInt(3, 4);
                    short point = (short) Util.nextInt(1, tn); // Đảm bảo point không bao giờ là 0
                    if (type == 3) { // Nếu là giáp
                        point = (short) Math.max(1, point / 10); // Đảm bảo point không nhỏ hơn 1
                    }
                    this.nPoint.increasePoint(type, point, false); // Áp dụng cho đệ
                } else {
                    // Tăng chỉ số 0, 1 hoặc 2 (HP, KI hoặc sức đánh)
                    byte type = (byte) Util.nextInt(0, 2);// Đảm bảo point không bao giờ là 0
                    this.nPoint.increasePoint(type, (short) 1, false);
                    this.nPoint.increasePoint(type, (short) 10, false);
                }
    ...
    public boolean canAttack() {
        ...
        if (this.master.isPl() && this.master.doesNotAttack && this.master.charms.tdDeTu < System.currentTimeMillis()) {
            if (Util.canDoWithTime(lastTimeAskAttack, 10000)) {
                Service.gI().chatJustForMe(master, this,
                        this.typePet == 1 ? "Sao ngươi không đánh đi?" : "Sao sư phụ không đánh đi?");
            }
            return false;
        }
        return true;
