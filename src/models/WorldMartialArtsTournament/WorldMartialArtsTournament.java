...
public final class WorldMartialArtsTournament implements Runnable {
    ...
    private void leaveMap() {
        try {
            if (player_2 == null || player_2.zone == null || !player_2.zone.equals(zone)
                    || player_2.zone.map.mapId != 51) {
                plWin = player_1;
                plLose = player_2;
                typeEnd = 3;
                finish();
            } else if (player_1 == null || player_1.zone == null || !player_1.zone.equals(zone)
                    || player_1.zone.map.mapId != 51) {
                plWin = player_2;
                plLose = player_1;
                typeEnd = 3;
                finish();
    ...
    public void finish() {
        try {
            thongBao();
            if (plWin != null) {
                if (WorldMartialArtsTournamentManager.gI().listWait.isEmpty()
                        && WorldMartialArtsTournamentManager.gI().listTournaments.size() == 1) {
    ...
    private void npcChat() {
        switch (typeEnd) {
            ...
            case 3 -> {
                Service.gI().chat(npc,
                        ConstTournament.TEXT_NPC_CHAT_DOI_THU_BO_CUOC_ROI_MAP.replaceAll("%1", plWin.name));
            }
        }
    }
