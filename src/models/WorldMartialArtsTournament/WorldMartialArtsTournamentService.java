...
public class WorldMartialArtsTournamentService extends ConstTournament {
    ...
    public static String sayText() {
        return WorldMartialArtsTournamentManager.gI().canReg
                ? "Chào mừng bạn đến với đại hội võ thuật\nGiải " + tournamentNames[getTournament()] + " đang có "
                        + WorldMartialArtsTournamentManager.gI().listReg.size() + " người đăng ký thi đấu"
    ...
    public static void menu(Npc npc, Player player) {
        if (WorldMartialArtsTournamentManager.gI().round != 0
                && WorldMartialArtsTournamentManager.gI().checkPlayer(player.id)) {
        ...
        switch (player.iDMark.getIndexMenu()) {
            ...
            case ConstNpc.BASE_MENU -> {
                switch (select) {
                    case 0 ->
                        NpcService.gI().createTutorial(player, npc.tempId, npc.avartar,
                                ConstNpc.THONG_TIN_DAI_HOI_VO_THUAT);
                    case 1 -> {
                        if (canReg && tour != -1) {
                            if (!regCheck(player)) {
                                ArrayList<String> menu = new ArrayList<>();
                                ...
                                menu.add("Từ chối");
                                npc.createOtherMenu(
                                        player, ConstNpc.DANGKYDHVT_CONFIRM, "Hiện đang có giải đấu "
                                                + tournamentNames[tour] + " bạn có muốn đăng ký không?",
    ...
    public static void dangky_huy(Npc npc, Player player) {
        ...
        if (!WorldMartialArtsTournamentManager.gI().listReg.contains(player.id)) {
            ...
            // NpcService.gI().createTutorial(player, npc.tempId, npc.avartar, "Bạn không đủ
            // vàng, còn thiếu " + (gold - player.inventory.gold) + " vàng nữa");
            // return;
            // }
            Item thoiVang = InventoryService.gI().findItemBag(player, 457);
            if (thoiVang == null) {
                NpcService.gI().createTutorial(player, npc.tempId, npc.avartar,
                        "Bạn không đủ thỏi vàng, còn thiếu " + (gold) + " thỏi vàng nữa");
                return;
            }
            if (thoiVang.quantity < gold) {
                NpcService.gI().createTutorial(player, npc.tempId, npc.avartar,
                        "Bạn không đủ thỏi vàng, còn thiếu " + (gold - thoiVang.quantity) + " thỏi vàng nữa");
                return;
            }
            if (player.inventory.getGemAndRuby() < gem) {
                NpcService.gI().createTutorial(player, npc.tempId, npc.avartar,
                        "Bạn không đủ ngọc, còn thiếu " + (gem - player.inventory.getGemAndRuby()) + " ngọc nữa");
                return;
            }
            ...
            NpcService.gI().createTutorial(player, npc.tempId, npc.avartar,
                    ConstTournament.TEXT_DANG_KY_THANH_CONG.replaceAll("%1", TimeUtil.getCurrHour() + "")
                            .replaceAll("%2", TimeUtil.getCurrHour() + "h" + TimeUtil.getCurrMin()));
        } else {
