...
public class WorldMartialArtsTournamentManager implements Runnable {
    ...
    public void updateChatText() {
        ...
        if (canReg) {
            chatText.add("Bạn hãy nhanh chân đăng ký ngay bây giờ, giải đấu sẽ bắt đầu vào lúc "
                    + TimeUtil.getCurrHour() + "h30");
        } else if (TimeUtil.getCurrMin() < ConstTournament.MINS_START) {
            chatText.add("Giải đấu tiếp theo sẽ diễn ra vào lúc "
                    + WorldMartialArtsTournamentService.getNextTournamentTime() + "h");
        } else {
            chatText.add("Đang trong thời gian thi đấu, xin chờ đến "
                    + WorldMartialArtsTournamentService.getNextTournamentTime() + "h để đăng ký");
        }
    }
    ...
    public void update() {
        if (TimeUtil.getCurrMin() > ConstTournament.MINS_END) {
            ...
            if (round > 0) {
                if (listWait.size() > 1 && listTournaments.isEmpty()
                        && waitTime - (System.currentTimeMillis() - lastWaitTime) > 30000) {
                    lastWaitTime = System.currentTimeMillis();
                    waitTime = 30000;
                    sendWaitNotify();
                }
                if (listWait.size() > 1 && listReg.isEmpty() && listTournaments.isEmpty()
                        && Util.canDoWithTime(lastWaitTime, waitTime)) {
            ...
            if (lastMins != TimeUtil.getCurrMin()) {
                ...
                for (int i = listReg.size() - 1; i >= 0; i--) {
                    try {
                        Player pl = getPlayerById(listReg.get(i));
                        if (pl != null && pl.zone != null) {
                            Service.gI().sendThongBao(pl, "Trận đấu của bạn sẽ diễn ra trong vòng "
                                    + (ConstTournament.MINS_START - TimeUtil.getCurrMin()) + " phút nữa");
                        }
                    } catch (Exception e) {
                    }
                }
        ...
        if (!listWait.isEmpty()) {
            for (int i = listWait.size() - 1; i >= 0; i--) {
                try {
                    ...
                    if (pl != null && pl.zone != null) {
                        Service.gI().sendThongBao(pl, "Trận đấu của bạn sẽ diễn ra trong vòng "
                                + TimeUtil.getTimeLeft(lastWaitTime, waitTime / 1000) + " nữa");
                    }
                } catch (Exception e) {
                }
            }
    ...
    public void reward(Player pl) {
        ...
        String msg = ConstTournament.TEXT_KHOE_VO_DICH.replaceAll("%1", pl.name).replaceAll("%2",
                ConstTournament.tournamentNames[WorldMartialArtsTournamentService.getTournament()]);
