    ...
    public void claimOrDel(Player pl, byte action, int id) {
        ...
        switch (action) {
            ...
            case 2: // nhận tiền
                ...
                // Nếu giá là thỏi vàng
                if (it.goldSell > 0) {
                    int soLuongNhan = it.goldSell - (it.goldSell * 5 / 100); // trừ 5% phí
                    Item tv = ItemService.gI().createNewItem((short) 457, soLuongNhan);
                    InventoryService.gI().addItemBag(pl, tv);
                    InventoryService.gI().sendItemBag(pl);
                    Service.gI().sendThongBao(pl, "Bạn đã nhận được " + soLuongNhan + " thỏi vàng sau khi trừ 5% phí");
                } // Nếu giá là ngọc hồng
                else if (it.gemSell > 0) {
                    int soLuongNhan = it.gemSell - (it.gemSell * 5 / 100);
                    pl.inventory.gem += soLuongNhan;
                    Service.gI().sendMoney(pl);
                    Service.gI().sendThongBao(pl, "Bạn đã nhận được " + soLuongNhan + " hồng ngọc sau khi trừ 5% phí");
                }

                // Xóa vật phẩm khỏi danh sách ký gửi
                if (ConsignShopManager.gI().listItem.remove(it)) {
                    openShopKyGui(pl);
                }
                break;
