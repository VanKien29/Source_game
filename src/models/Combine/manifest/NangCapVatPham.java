...
public class NangCapVatPham {
    ...
    private static float getRatio(int level) {
        return switch (level) {
            case 0 ->
                70f;
            case 1 ->
    ...
    public static void showInfoCombine(Player player) {
        ...
        for (Item item : player.combine.itemsCombine) {
            if (item == null || !item.isNotNullItem() || item.template == null) {
                Service.gI().sendDialogMessage(player,
                        "Vật phẩm trong khung nâng cấp bị lỗi (template null). Hãy bỏ ra và thử lại.");
                return;
            }
        ...
        if (trangBi == null || trangBi.template == null || daNangCap == null || daNangCap.template == null
                || !trangBi.canNangCapWithNDC(daNangCap)) {
            Service.gI().sendDialogMessage(player, "Cần 1 trang bị và đúng loại đá nâng cấp");
            return;
        }
        ...
        String infoUp = trangBi.getOptionInfoUpgrade();
        String infoUpFinal = trangBi.getOptionInfoUpgradeFinal();

        text.append(ConstFont.BOLD_DARK).append(infoUp != null ? infoUp : "").append("\n");
        text.append(ConstFont.BOLD_BLUE).append("Sau khi nâng cấp [+").append(level + 1).append("]\n");
        text.append(ConstFont.BOLD_GREEN).append(infoUpFinal != null ? infoUpFinal : "").append("\n");
        text.append(ConstFont.BOLD_BLUE).append("Tỉ lệ thành công: ").append(getRatio(level)).append("%\n");

        text.append(daNangCap.quantity >= da ? ConstFont.BOLD_BLUE : ConstFont.BOLD_RED)
                .append("Cần ").append(da).append(" ").append(daNangCap.template.name).append("\n");
