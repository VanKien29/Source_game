...
public class NangCapBongTai3 {
    ...
    public static void showInfoCombine(Player player) {
        if (player.combine.itemsCombine.size() != 2) {
            Service.gI().sendDialogMessage(player, "Cần 1 bông tai cấp 2 và 9999 mảnh vỡ bông tai cấp 3.");
            return;
        }
        Item bongTai = null;
        Item manhVo = null;
        ...
        text.append(quantityManhVo >= 9999 ? ConstFont.BOLD_BLUE : ConstFont.BOLD_RED).append("Cần 9999 Mảnh vỡ bông tai cấp 3\n");
        ...
        text.append(ConstFont.BOLD_RED).append("Thất bại -999 mảnh vỡ bông tai\n");
    ...
    public static void nangCapBongTai3(Player player) {
        ...
        if (Util.isTrue(20, 100)) {
            Item btc3 = ItemService.gI().createNewItem((short) 1774);
            btc3.itemOptions.add(new Item.ItemOption(72, 3));
            btc3.itemOptions.add(new Item.ItemOption(30, 1));
            btc3.itemOptions.add(new Item.ItemOption(38, 1));
            InventoryService.gI().subQuantityItemsBag(player, bongTai, 1);
            InventoryService.gI().addItemBag(player, btc3);
            CombineService.gI().sendEffectSuccessCombine(player);
            InventoryService.gI().subParamItemsBag(player, 1772, 31, 9999);
        } else {
            CombineService.gI().sendEffectFailCombine(player);
            InventoryService.gI().subParamItemsBag(player, 1772, 31, 999);
        }
