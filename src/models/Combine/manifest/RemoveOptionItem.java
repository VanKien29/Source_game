            ...
            if (player.combine.itemsCombine.size() == 2) {
                ...
                if (itemHacHoa == null) {
                    CombineService.gI().baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU, 
                            "Cần trang bị đã Pháp sư hóa", "Đóng");
                    return;
                }
                ...
                npcSay += "|2|Sau khi tẩy sẽ xóa các chỉ số pháp sư:\n" +
                          "Tấn công, HP, KI, Giáp, Xóa luôn cả NYC của bạn\n" +
                          "|7|Chỉ áp dụng cho trang bị là: Pet, Đeo lưng\n" +
        ...
        if (InventoryService.gI().getCountEmptyBag(player) > 0) {
            ...
            if (type == 11 || type == 21 || type == 72) {
                if (Util.isTrue(35, 100)) {
                    ...
                    Service.gI().sendThongBao(player, "Tẩy thất bại, đồ không bị mất chỉ số!");
                }
            } else {
                Service.gI().sendThongBao(player, "Chỉ có thể tẩy trang bị pháp sư hóa");
            }
