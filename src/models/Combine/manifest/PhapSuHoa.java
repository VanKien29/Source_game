...
//            }
            ...
            if (Util.isTrue(player.combine.ratioCombine, 100)) {
                ...
                List<Integer> idOptionHacHoa = Arrays.asList(0, 6, 7);
                int randomOption = idOptionHacHoa.get(Util.nextInt(0, idOptionHacHoa.size() - 1));
                ...
                } else if (randomOption == 6 || randomOption == 7) {
                    addValue = Util.nextInt(5000, 20000);
                } else {
                    addValue = 0;
                }
                if (!trangBiHacHoa.haveOption(randomOption)) {
                    trangBiHacHoa.itemOptions.add(new ItemOption(randomOption, addValue));
                } else {
                    for (ItemOption itopt : trangBiHacHoa.itemOptions) {
                        if (itopt.optionTemplate.id == randomOption) {
                            itopt.param += addValue;
                            break;
                        }
                    }
                }
