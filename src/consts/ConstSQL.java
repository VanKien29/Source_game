...
public class ConstSQL {
    ...
    public static final String TOP_DUA_NAP = """
    SELECT name, gender, items_body, CAST(danap AS UNSIGNED) AS danap
    FROM account, player
    WHERE account.id = player.account_id
      AND account.danap >= 10000
      AND account.is_admin = 0 
...
""";
    ...
    public static final String TOP_NAP = """
    SELECT name, gender, items_body,
           CAST(cash AS UNSIGNED) AS cash
    FROM account, player
    WHERE account.id = player.account_id
      AND account.is_admin = 0
    ORDER BY cash DESC
...
//    public static final String TOP_DUA_NAP = "SELECT name, gender, items_body, CAST( danap AS UNSIGNED) AS danap FROM account, player WHERE account.id = player.account_id AND account.danap >= 10000 ORDER BY account.danap DESC LIMIT 10;";
    ...
    public static final String TOP_WHIS = "SELECT name, player.id, gender, items_body, CAST( JSON_EXTRACT(data_luyentap, '$[5]') AS UNSIGNED) AS top, CAST( JSON_EXTRACT(data_luyentap, '$[6]') AS UNSIGNED) AS time, CAST( JSON_EXTRACT(data_luyentap, '$[7]') AS UNSIGNED) AS lasttime FROM player INNER JOIN account ON account.id = player.account_id WHERE account.ban = 0 AND CAST( JSON_EXTRACT(data_luyentap, '$[5]') AS UNSIGNED) > 10 ORDER BY CAST( JSON_EXTRACT(data_luyentap, '$[5]') AS UNSIGNED) DESC, CAST( JSON_EXTRACT(data_luyentap, '$[6]') AS UNSIGNED) ASC LIMIT 20;";
    public static final String TOP_3_WHIS = "SELECT name, id, gender, items_body, CAST( JSON_EXTRACT(data_luyentap, '$[5]') AS UNSIGNED) AS top, CAST( JSON_EXTRACT(data_luyentap, '$[6]') AS UNSIGNED) AS time, CAST( JSON_EXTRACT(data_luyentap, '$[7]') AS UNSIGNED) AS lasttime FROM player INNER JOIN account ON account.id = player.account_id WHERE account.ban = 0 AND CAST( JSON_EXTRACT(data_luyentap, '$[5]') AS UNSIGNED) > 10 ORDER BY CAST( JSON_EXTRACT(data_luyentap, '$[5]') AS UNSIGNED) DESC, CAST( JSON_EXTRACT(data_luyentap, '$[6]') AS UNSIGNED) ASC LIMIT 3;";
    public static final String TOP_PHAO_BONG = "SELECT name, gender, items_body, phaobong FROM player INNER JOIN account ON account.id = player.account_id WHERE account.ban = 0 ORDER BY phaobong DESC LIMIT 10;";
    public static final String TOP_LIXI = "SELECT name, gender, items_body, lixi FROM player INNER JOIN account ON account.id = player.account_id WHERE account.ban = 0 ORDER BY lixi DESC LIMIT 10;";
    public static final String TOP_SB = "SELECT name, gender, items_body, event_point_boss FROM player INNER JOIN account ON account.id = player.account_id WHERE account.ban = 0 ORDER BY event_point_boss DESC LIMIT 10;";
    public static final String TOP_SO_SU_MENH = "SELECT *, CAST(JSON_UNQUOTE(JSON_EXTRACT(so_su_menh, '$.point')) AS SIGNED) AS point_value FROM player ORDER BY point_value DESC LIMIT 10;";
