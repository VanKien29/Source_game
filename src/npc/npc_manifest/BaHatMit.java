...
import static models.Combine.CombineService.TAY_PS_HOA_TRANG_BI;
            ...
            switch (this.mapId) {
                case 5 ->
                    this.createOtherMenu(player, ConstNpc.BASE_MENU, "Ngươi tìm ta có việc gì?", "Chức năng\nPha lê",
                            "Nâng cấp\nTrang bị", "Võ đài\nSinh Tử", "Nâng cấp Chân mệnh", "Đổi Vé\nHủy Diệt", "Chức Năng khác");
                case 112 -> {
                ...
                default -> {
                    ...
                    if (InventoryService.gI().findItem(player, 454)
                            ...
                            || InventoryService.gI().findItem(player, 1774)) {
                        menu = new ArrayList<>(Arrays.asList(
                                "Sách\nTuyệt Kỹ",
                                "Cửa hàng\nBùa",
                                "Nâng cấp\nVật phẩm",
                                // Nếu có Bông Tai C2
                                InventoryService.gI().findItemBongTaiCap2(player)
                                ? "Mở chỉ số\nBông tai\nPorata cấp\n2"
                                : "Nâng cấp\nBông tai\nPorata",
                                // Nếu có Bông Tai C3
                                InventoryService.gI().findItemBongTaiCap3(player)
                                ? "Mở chỉ số\nBông tai\nPorata cấp\n3"
                                : "Nâng cấp\nBông tai\nPorata cấp 3",
                                "Làm phép\nNhập đá",
                                "Nhập\nNgọc Rồng"));
                    ...
                    switch (player.iDMark.getIndexMenu()) {
                        case ConstNpc.BASE_MENU -> {
                            switch (select) {
                                ...
                                case 4 ->
                                    CombineService.gI().openTabCombine(player, CombineService.DOI_DO_THAN);
                                case 5 ->
