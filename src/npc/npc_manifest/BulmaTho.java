        ...
        if (canOpenNpc(player)) {
            ...
            this.createOtherMenu(player, ConstNpc.BASE_MENU,
                    "|0|Em muốn được tặng hoa , anh nào tặng hoa cho em đi\n|4|"
                            + "[Hoa hồng mọc dại quanh các làng, đảo , vách núi, ....]",
                    "Tặng hoa", "Tỉa hoa", "Phụ\nkiện");
            ...
            if (player.iDMark.isBaseMenu()) {
                switch (select) {
                    case 0 -> {
                        ...
                        int[] money = { utils.Util.nextInt(333333, 999999), utils.Util.nextInt(50, 100),
                                utils.Util.nextInt(200, 300) };
                        ...
                        InventoryService.gI().subQuantityItemsBag(player, boHoaHong2 == null ? boHoaHong1 : boHoaHong2,
                                1);
                        Item caiTrang = ItemService.gI()
                                .createNewItem((short) idCT[utils.Util.nextInt(0, idCT.length - 1)]);
                        Item thuCung = ItemService.gI()
                                .createNewItem((short) idPet[utils.Util.nextInt(0, idPet.length - 1)]);
                        Item deoLung = ItemService.gI()
                                .createNewItem((short) idDeoLung[utils.Util.nextInt(0, idDeoLung.length - 1)]);
                        Item vatPham = ItemService.gI()
                                .createNewItem((short) idVatPham[utils.Util.nextInt(0, idVatPham.length - 1)]);
                        if (utils.Util.isTrue(5, 100)) {
                            ...
                            caiTrang.addOptionParam(
                                    utils.Util.isTrue(5, 100) ? 117 : utils.Util.isTrue(5, 100) ? 14 : 5,
                                    utils.Util.nextInt(8, 12));
                            ...
                            thuCung.addOptionParam(utils.Util.isTrue(5, 100) ? 117 : utils.Util.isTrue(5, 100) ? 14 : 5,
                                    utils.Util.nextInt(3, 5));
                            if (utils.Util.isTrue(99, 100)) {
                    ...
                    case 1 -> {
                        ...
                        if (hoaHong.quantity > 10) {
                            ...
                            Item boHoaHong = ItemService.gI()
                                    .createNewItem((short) idBoHoaHong[utils.Util.nextInt(0, idBoHoaHong.length - 1)]);
                            InventoryService.gI().addItemBag(player, boHoaHong);
                            services.Service.gI().sendThongBao(player,
                                    " " + boHoaHong.template.name + " đẹp quá, anh có thể tặng em được không?");
                            services.Service.gI().sendMoney(player);
                            InventoryService.gI().sendItemBag(player);
                        } else {
