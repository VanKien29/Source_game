    ...
    public void confirmMenu(Player pl, int select) {
        if (canOpenNpc(pl)) {
            switch (select) {
                ...
                case 1 -> {
//                    if (pl.getSession().player.nPoint.power >= 80000000000L) {
                        if (pl.getSession().actived) {
                            ConsignShopService.gI().openShopKyGui(pl);
                        } else {
                            Service.gI().sendThongBao(pl, "Cần mở thành viên để sử dụng chức năng");
                        }
//                    } else {
//                        Service.gI().sendThongBao(pl, "Yêu cầu trên 80 tỷ sức mạnh");
//                    }
