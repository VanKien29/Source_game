            ...
            switch (this.mapId) {
                ...
                case 155 ->
                    this.createOtherMenu(player, ConstNpc.BASE_MENU, "Ta có thể giúp gì cho ngươi ?",
                            "Quay về", "Từ chối");
                case 184 ->
                    this.createOtherMenu(player, ConstNpc.BASE_MENU, "Mau hút năng lượng tà ác trong người cậu ấy",
                            "Về nhà", "Từ chối");
                case 52 -> {
                    player.fightMabu.clear();
                    boolean hasEnergyJar = InventoryService.gI().findItemBag(player, 1951) != null;
                    if (this.mapId == 52) {
                        if (TimeUtil.isMabu14HOpen()) {
                            ...
                            if (hasEnergyJar) {
                                this.createOtherMenu(player, ConstNpc.MENU_NOT_OPEN_MMB,
                                        "Vào lúc " + MajinBuuService.HOUR_OPEN_MAP_MABU
                                        + "h tôi sẽ bí mật...\nđuổi theo 2 tên đồ tể...\nQuý vị nào muốn đi theo thì xin mời !",
                                        "OK", "Bình hút năng lượng", "Từ chối");
                            } else {
                                this.createOtherMenu(player, ConstNpc.MENU_NOT_OPEN_MMB,
                ...
                case 127 -> {
                    String npcSay = player.isPhuHoMapMabu ? "Ta có thể giúp gì cho ngươi ?"
            ...
            switch (this.mapId) {
                    ...
                    switch (player.iDMark.getIndexMenu()) {
                        case ConstNpc.MENU_OPEN_MMB -> {
                            if (select == 0) {
                                ...
                                boolean hasEnergyJar = InventoryService.gI().findItemBag(player, 1951) != null;
                                if (!hasEnergyJar) {
                                    Service.gI().sendThongBao(player, "Bạn chưa có Bình hút năng lượng");
                                    return;
                                }
                                this.createOtherMenu(player, ConstNpc.BINH_HUT_NANG_LUONG,
                        ...
                        case ConstNpc.MENU_NOT_OPEN_MMB -> {
                            if (select == 0) {
                                Service.gI().sendThongBao(player, "Chưa đến giờ mở cửa vào hang Mabu.");
                            } else if (select == 1) {
                                boolean hasEnergyJar = InventoryService.gI().findItemBag(player, 1951) != null;
                                if (!hasEnergyJar) {
                                    Service.gI().sendThongBao(player, "Bạn chưa có Bình hút năng lượng");
                        ...
                        case ConstNpc.BINH_HUT_NANG_LUONG -> {
                            if (select == 0) {
                                ChangeMapService.gI().changeMap(player, 184, -1, Util.nextInt(100, 500), 312);
                            }
                        }
                    }
                }
                case 114, 115, 117, 118, 119, 120 -> {
                    ...
                    switch (select) {
                        ...
                        case 1 -> {
                            if (!player.itemTime.isUseGTPT) {
                                ...
                                Service.gI().sendThongBao(player,
                                        "Phép thuật đã được giải trừ, sức đánh của bạn đã tăng theo điểm tích lũy");
                        ...
                        case 2 -> {
                            if (!player.itemTime.isUseGTPT && player.fightMabu.pointMabu >= player.fightMabu.POINT_MAX
                                    && this.mapId != 120) {
                                ChangeMapService.gI().changeMap(player, this.map.mapIdNextMabu((short) this.mapId), -1,
                                        this.cx, this.cy);
                            }
