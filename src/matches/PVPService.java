...
public class PVPService {
    ...
    public void sendInvitePVP(Player pl, byte selectGold) {
        ...
        try {
            ...
            msg.writer().writeUTF(pl.name + " (sức mạnh " + Util.numberToMoney(pl.nPoint.getFullTN())
                    + ") muốn thách đấu bạn với mức cược " + goldThachDau);
    ...
    public void sendInvitePVP(Player pl, Player plMap) {
        ...
        if (Service.gI().getCurrLevel(pl) != Service.gI().getCurrLevel(plMap)) {
            Service.gI().sendThongBao(pl,
                    "Luyện tập cùng đối thủ ở cấp " + Service.gI().getCurrStrLevel(pl) + " để đạt hiệu quả tốt nhất");
        }
        ...
        try {
            ...
            msg.writer().writeUTF(
                    pl.name + " (sức mạnh " + Util.numberToMoney(pl.nPoint.power) + ") muốn luyện tập với bạn");
            plMap.sendMessage(msg);
            msg.cleanup();
        } catch (Exception e) {
        }
    ...
    public void openSelectRevenge(Player pl, long idEnemy) {
        ...
        NpcService.gI().createMenuConMeo(pl, ConstNpc.REVENGE,
                -1, "Bạn muốn đến ngay chỗ hắn, phí là 1 ngọc\nvà được tìm thoải mái trong 5 phút nhé", "OK",
                "Từ chối");
    }
