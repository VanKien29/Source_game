...
//                    System.out.println("moveTo");
    ...
    public void reward(Player plKill) {
        if (goldAnTrom != 0) {
            // random số lượng từ 5 đến 10
            int soLuong = Util.nextInt(5, 20);

            for (int i = 0; i < soLuong; i++) {
                ItemMap it = new ItemMap(this.zone,457, 1, this.location.x + i * 3, this.zone.map.yPhysicInTop(this.location.x, this.location.y - 24),
                        plKill.id
                );
                Service.gI().dropItemMap(this.zone, it);
            }
        }
        // Tăng tiến độ danh hiệu 1296 (Mốc sách túi) khi hạ Ăn Trộm
        if (plKill != null && plKill.playerTask != null && plKill.playerTask.taskdh != null) {
            if (plKill.playerTask.taskdh.MocSachTui < 20) {
                plKill.playerTask.taskdh.MocSachTui++;
                plKill.playerTask.taskdh.ResetTime = System.currentTimeMillis();
                int percentDone = (int) ((double) plKill.playerTask.taskdh.MocSachTui / 20 * 100);
                // Chỉ thông báo nếu chưa ạt 100%
                if (percentDone < 100) {
                    Service.gI().sendThongBao(plKill, "Tiến độ Mốc Sách Túi:  " + percentDone + "%");
                }
            }
        }
    }
