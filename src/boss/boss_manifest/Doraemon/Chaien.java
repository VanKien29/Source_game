...
public class Chaien extends Boss {
    ...
    public Chaien() throws Exception {
        super(BossID.CHAIEN, false, true, BossesData.CHAIEN);
    }
    ...
    public void reward(Player plKill) {
        ...
        if (Util.isTrue(75, 100)) {
            ItemMap it = new ItemMap(this.zone, 1724, Util.nextInt(1, 5), this.location.x, this.zone.map.yPhysicInTop(this.location.x,
    ...
    public void doneChatE() {
        ...
        for (Boss boss : this.parentBoss.bossAppearTogether[this.parentBoss.currentLevel]) {
            if ((boss.id == BossID.DORAEMON || boss.id == BossID.XUKA || boss.id == BossID.XEKO || boss.id == BossID.NOBITA) && !boss.isDie()) {
                return;
            }
        }
