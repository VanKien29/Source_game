...
public class Boss extends Player implements IBoss, IBossOutfit {
    ...
    public boolean chatE() {
        if (Util.canDoWithTime(lastTimeChatE, timeChatE)) {
            ...
            if (texts == null || texts.length == 0) {
                return true; // không có gì để chat
            }

            int currentIndex = this.indexChatE;
            if (currentIndex >= texts.length) {
                return true;
            }

            String textChat = texts[currentIndex];
            int prefix = Integer.parseInt(textChat.substring(1, textChat.lastIndexOf("|")));
            textChat = textChat.substring(textChat.lastIndexOf("|") + 1);
    ...
    public void setRuntimeDisabled(boolean disabled) {
        ...
        if (disabled) {
            ...
            this.lastTimeRest = System.currentTimeMillis();
            this.changeStatus(BossStatus.REST);
        } else if (this.zone == null && this.bossStatus == BossStatus.REST) {
            this.changeStatus(BossStatus.RESPAWN);
        }
    }
