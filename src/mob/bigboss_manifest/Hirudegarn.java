    ...
    public void update() {
        ...
        if (isDie() && (System.currentTimeMillis() - lastTimeDie) > 600000 && lvMob == 3) {
            ...
            if (Util.isTrue(1, 100)) {
                ItemMap it = new ItemMap(this.zone, 1951, 1, this.location.x, this.zone.map.yPhysicInTop(this.location.x, this.location.y - 24), -1);
                it.options.add(new ItemOption(255, 0));
                Service.gI().dropItemMap(this.zone, it);
            }
            Service.gI().sendBigBoss2(this.zone, action, this);
            if (lvMob <= 2) {
