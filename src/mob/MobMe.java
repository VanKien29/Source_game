...
public final class MobMe extends Mob {
        ...
        try {
            ...
            if (mob != null) {
                if (mob.point.gethp() > this.point.dame) {
                    ...
                    msg.writeLongByHoandz(Util.maxIntValue(mob.point.gethp()), cn.readInt);
                    msg.writeLongByHoandz(Util.maxIntValue(this.point.dame), cn.readInt);
                    Service.gI().sendMessAllPlayerInMap(this.player, msg);
                    msg.cleanup();
    ...
    public void spawn() {
        ...
        try {
            ...
            msg.writeLongByHoandz(Util.maxIntValue(this.point.hp), cn.readInt);// hp mob
            Service.gI().sendMessAllPlayerInMap(this.player, msg);
            msg.cleanup();
        } catch (Exception e) {
        }
    ...
    public synchronized void injured(Player plAtt, long damage, boolean dieWhenHpFull) {
        ...
        try {
            ...
            msg.writeLongByHoandz(Util.maxIntValue(point.hp), cn.readInt);
            Service.gI().sendMessAllPlayerInMap(this.player, msg);
            msg.cleanup();
            if (isDie()) {
                mobMeDie();
