...
import network.server.HOANDZServer;
import network.server.HoandzSessionManager;
...
//                if (HOANDZServer.firewall.containsKey(ip)) {
//                    int count = HOANDZServer.firewall.get(ip);
//                    if (count > 0) {
//                        HOANDZServer.firewall.put(ip, count - 1);
//                    } else {
//                        HOANDZServer.firewall.remove(ip);
//                    }
//                }
    ...
    public void dispose() {
        ...
        this.ip = null;
        /* 190 */
        HoandzSessionManager.gI().removeSession(this);
    }
    ...
    public void sendKey() throws Exception {
        ...
        /* 198 */
        if (HOANDZServer.gI().isRandomKey()) {
            /* 199 */
    ...
    public void initThreadSession() {
        this.tSender = new Thread((this.sender != null) ? (Runnable) this.sender.setSocket(this.socket)
                : (Runnable) (this.sender = new Sender(this, this.socket)), "Thread tsender");
        this.tCollector = new Thread((this.collector != null) ? (Runnable) this.collector.setSocket(this.socket)
                : (Runnable) (this.collector = new Collector(this, this.socket)), "Thread collecter");
    }
}
