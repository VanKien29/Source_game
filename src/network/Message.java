...
public class Message
    ...
    public void cleanup() {
        try {
            // Đóng tất cả tài nguyên trong một vòng lặp để tránh lặp lại code
            for (Closeable closeable : new Closeable[]{this.is, this.os, this.dis, this.dos}) {
                if (closeable != null) {
                    closeable.close();
                }
            }
        } catch (IOException ignored) {
            // Bỏ qua lỗi mà không cần ghi log để giữ cho code mượt
        }
    }
    ...
    public void writeLongByHoandz(long v, boolean is) throws IOException {
        if (is) {
            this.writeInt((int) v);
            return;
        }
